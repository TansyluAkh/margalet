...
def new_user(chatid, letters, open, pos, r1, r2, r3, r4, r5):
    print('new')
    cur = con.cursor()
    cur.execute('SELECT 1 FROM players WHERE id=%s',(chatid,))
    s = cur.fetchone()
    print(s)
    if s == None:
        print('INSERT')
        cur.execute("INSERT INTO players (id, letters,\
         open, pos, r1, r2, r3, r4, r5) VALUES (%s, %s, %s, \
         %s, %s, %s, %s, %s, %s)", (chatid,letters, open, pos, r1, r2, r3, r4, r5))
    else:
...
def new_score(chatid, score):
    cur = con.cursor()
    s = cur.execute('SELECT 1 FROM users WHERE id=%s',(str(chatid)))
    if s != 1:
...
def get_user(chatid):
    cur = con.cursor()
    cur.execute('SELECT letters, open, pos, r1, r2, r3, r4, r5 FROM players WHERE id={}'.format(str(chatid)))
    arr = cur.fetchone()
    print(arr)
    if arr != None:
        info = User( *list(arr))
